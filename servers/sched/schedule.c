...
int do_noquantum(message *m_ptr)
{
	...
	// 1st we commented the multi level algorithm of scheduling used by MINIX3

	/*if (rmp->priority < MIN_USER_Q) {
		rmp->priority += 1; // lower priority 
	}*/

	/*2nd we implemented 4 different algorithms which the user will choose from as follows
	* 
	* 1:Round Robin
	...
	*/
	...
	//2:shortest job first
		...
		for (int i = 0; i < NR_PROCS; i++) {// for loop checking all processes to get the shortest of them
	...
	//3:priority
		...
		for (int i = 0; i < NR_PROCS; i++) {// for loop checking all prcesses for highest priority
			schedproc currentProc = schedproc[i];
			if (currentProc->in_queue_priority < minPrio && currentProc->flags & IN_USE) {
	...
	//4:Multi level feedback
		...
		switch (rmp->priority) {// switch for giving the process the suitable scheduling queue depending on the queue priority
		...
		case (USER_Q + 3)://level4
			...
			for (int i = 0; i < NR_PROCS; i++) {// for loop checking all processes to get the shortest of them
				...
				if (currentProc->expected_job_time < minBurst && currentProc->flags & IN_USE && currentProc->priority == USER_Q + 3) {// check only processes in this level
					minBurst = currentProc->expected_job_time;
					minProcess = currentProc;
				}
			}
...
int do_start_scheduling(message *m_ptr)
{
	...
	rmp->in_queue_priority = rand() % 10; // random priority from 0- 9;
	rmp->expected_job_time = rand() % 900 + 1; // set random expected time from 1 - 900
